...
But what if we want to exclude certain islands below a certain size. Because that is not a variable in the dataframe from LMÍ we could calculate the are from within R:
...
OK, so there as still a few invalids. Lets give it a one more go:
...
Ok, so by this process all simple features are valid. Lets make the object "permanently" valid (and get rid of variables we do nt really need):
